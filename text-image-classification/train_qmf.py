...
def get_optimizer(model: nn.Module, args: argparse.Namespace) -> optim.Optimizer:
    if args.model in ["bert", "concatbert", "mmbt"]:
        total_steps = (args.train_data_len / args.batch_sz / args.gradient_accumulation_steps * args.max_epochs)
        ...
        optimizer = BertAdam(optimizer_grouped_parameters, lr=args.lr, warmup=args.warmup, t_total=total_steps)
    else:
...
def rank_loss(confidence, indices, history):
    ...
    ranking_loss = nn.MarginRankingLoss(margin=0.0)(confidence1, confidence2, -rank_target.reshape(-1, 1))
    return ranking_loss


def model_forward(epoch, model, args, criterion, batch, txt_history=None, img_history=None, mode='train'):
    ...
    if args.model == "bow":
        ...
        text_img_logits, text_logits, image_logits, text_conf, image_conf = model(text, mask, segment, image)
...
def train(args: argparse.Namespace) -> None:
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Training epoch {epoch}"):
            loss, _, _ = model_forward(epoch, model, args, criterion, batch, txt_history, img_history, mode='train')
